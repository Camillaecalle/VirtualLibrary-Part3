...
//Date: March 4, 2023
//ArrayBag.cpp
//This file implements the two functions declared in my hpp file (operator += and operator /=).
...
//****************************************Project 3 functions added below****************************************************************//
...
*/
template<class ItemType>
void ArrayBag<ItemType>::operator+=(const ArrayBag<ItemType>& array_bag){
   if (array_bag.getCurrentSize() + item_count_ <= 10){ 
   for(int i = 0; i < array_bag.getCurrentSize(); i++){
      add(array_bag.items_[i]);
      }
   }
}
...
void ArrayBag<ItemType>::operator/=(const ArrayBag<ItemType>& array_bag){
   if(array_bag.getCurrentSize() + item_count_ <= 10){
      for(int i = 0; i < array_bag.getCurrentSize(); i++){
         if(contains(array_bag.items_[i]) == false) {
            add(array_bag.items_[i]);
         }
      }
   }
...
//****************************************Project 3 functions added above****************************************************************//
