...
//Date: March 4, 2023
//Book.cpp
//This file is is the Book Class and implements task 1's functions operator ==, operator =+, and display.
...
bool Book::operator==(const Book& book) const {
  if(getTitle() == book.title_ && getAuthor() == book.author_ && getPageCount() == book.page_count_ && isDigital() == book.is_digital_)
    return true;
  return false;
}
/**
...
void Book::display() const {
  if (is_digital_ == true){
  std::cout << getTitle() << " is written by " << getAuthor() << ". Page Count: " << getPageCount() << ". It is available digitally.\n";
   }  else {
   std::cout << getTitle() << " is written by " << getAuthor() << ". Page Count: " << getPageCount() << ". It is not available digitally.\n";
    }
}
